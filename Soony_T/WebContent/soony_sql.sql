...
insert into faq values(f_num.nextval,'[일반] 콩순이 유모차 주행 시 아이를 햇빛이나 미세먼지로부터 보호하는 가장 좋은 방법은 무엇인가요?','콩순이 유모차의 자동 썬 캐노피는 탑승하는 아이를 전체적으로 감싸주게 되어있으며 사용자의 버튼 조작으로 자동으로 동작 합니다. 쿨러기능을 추가하여 유모차 내부의 온도를 쉽게 낮출 수 있습니다.');
